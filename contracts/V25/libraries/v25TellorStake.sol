...
/**
* itle v25Tellor Stake
* @dev Contains the methods related to miners staking and unstaking. v25Tellor.sol
* references this library for function's logic.
*/

library v25TellorStake {
    ...
    */
    function requestStakingWithdraw(v25TellorStorage.TellorStorageStruct storage self) public {
        v25TellorStorage.StakeInfo storage stakes = self.stakerDetails[msg.sender];
        ...
        v25TellorDispute.updateMinDisputeFee(self);
    ...
    */
    function withdrawStake(v25TellorStorage.TellorStorageStruct storage self) public {
        v25TellorStorage.StakeInfo storage stakes = self.stakerDetails[msg.sender];
    ...
    */
    function depositStake(v25TellorStorage.TellorStorageStruct storage self) public {
    ...
    */
    function newStake(v25TellorStorage.TellorStorageStruct storage self, address staker) internal {
        require(v25TellorTransfer.balanceOf(self, staker) >= self.uintVars[keccak256("stakeAmount")], "Balance is lower than stake amount");
        ...
        self.uintVars[keccak256("stakerCount")] += 1;
        self.stakerDetails[staker] = v25TellorStorage.StakeInfo({
    ...
    */
    function getNewCurrentVariables(v25TellorStorage.TellorStorageStruct storage self) internal view returns(bytes32 _challenge,uint[5] memory _requestIds,uint256 _difficulty, uint256 _tip){
        for(uint i=0;i<5;i++){
    ...
    */
    function getNewVariablesOnDeck(v25TellorStorage.TellorStorageStruct storage self) internal view returns (uint256[5] memory idsOnDeck, uint256[5] memory tipsOnDeck) {
    ...
    */
    function getTopRequestIDs(v25TellorStorage.TellorStorageStruct storage self) internal view returns (uint256[5] memory _requestIds) {
        uint256[5] memory _max;
        uint256[5] memory _index;
        (_max, _index) = v25Utilities.getMax5(self.requestQ);
        for(uint i=0;i<5;i++){
            if(_max[i] != 0){
