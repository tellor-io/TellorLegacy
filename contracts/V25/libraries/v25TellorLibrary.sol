...
import "./v25TellorDispute.sol";
import "./v25TellorStake.sol";
import "./v25TellorGettersLibrary.sol";

/**
 * @title v25Tellor Oracle System Library
 * @dev Contains the functions' logic for the v25Tellor contract where miners can submit the proof of work
 * along with the value and smart contracts can requestData and tip miners.
 */
library v25TellorLibrary {
    ...
    */
    function addTip(v25TellorStorage.TellorStorageStruct storage self, uint256 _requestId, uint256 _tip) public {
        ...
        v25TellorTransfer.doTransfer(self, msg.sender, address(this), _tip);
    ...
    */
    function newBlock(v25TellorStorage.TellorStorageStruct storage self, string memory _nonce, uint256[5] memory _requestId) public {
        v25TellorStorage.Request storage _tblock = self.requestDetails[self.uintVars[_tBlock]];
        ...
        int256 _change = int256(v25SafeMath.min(1200, timeDiff));
        int256 _diff = int256(self.uintVars[difficulty]);
        _change = (_diff * (int256(self.uintVars[timeTarget]) - _change)) / 4000;
        if (_change == 0) {
                _change = 1;
            }
        self.uintVars[difficulty]  = uint256(v25SafeMath.max(_diff + _change,1));
        ...
        for (uint k = 0; k < 5; k++) {
            ...
            v25TellorStorage.Request storage _request = self.requestDetails[_requestId[k]];
        ...
        uint256[5] memory _topId = v25TellorStake.getTopRequestIDs(self);
        for(uint i = 0; i< 5;i++){
    ...
    */
    function submitMiningSolution(v25TellorStorage.TellorStorageStruct storage self, string calldata _nonce,uint256[5] calldata _requestId, uint256[5] calldata _value)
        external
    {
    ...
    function _submitMiningSolution(v25TellorStorage.TellorStorageStruct storage self, string memory _nonce,uint256[5] memory _requestId, uint256[5] memory _value)
        internal
    {
    ...
    function _verifyNonce(v25TellorStorage.TellorStorageStruct storage self,string memory _nonce ) view internal {
                require(uint256(
    ...
    * 
    */
    function _payReward(v25TellorStorage.TellorStorageStruct storage self, uint _timeDiff, address[5] memory miners) internal {
        ...
        v25TellorTransfer.doTransfer(self, address(this), miners[0], reward + _tip);
        v25TellorTransfer.doTransfer(self, address(this), miners[1], reward + _tip);
        v25TellorTransfer.doTransfer(self, address(this), miners[2], reward + _tip);
        v25TellorTransfer.doTransfer(self, address(this), miners[3], reward + _tip);
        v25TellorTransfer.doTransfer(self, address(this), miners[4], reward + _tip);
 
        //update the total supply
        self.uintVars[total_supply] +=  _devShare + reward * 5 - (self.uintVars[currentTotalTips] / 2);
        v25TellorTransfer.doTransfer(self, address(this), self.addressVars[_owner],  _devShare);
    ...
    */
    function proposeOwnership(v25TellorStorage.TellorStorageStruct storage self, address payable _pendingOwner) public {
    ...
    */
    function claimOwnership(v25TellorStorage.TellorStorageStruct storage self) public {
    ...
    */
    function updateOnDeck(v25TellorStorage.TellorStorageStruct storage self, uint256 _requestId, uint256 _tip) public {
        ...
        if(self.currentMiners[0].value == _requestId || self.currentMiners[1].value== _requestId ||self.currentMiners[2].value == _requestId||self.currentMiners[3].value== _requestId || self.currentMiners[4].value== _requestId ){
            ...
            if (_request.apiUintVars[requestQPosition] == 0) {
                uint256 _min;
                uint256 _index;
                (_min, _index) = v25Utilities.getMin(self.requestQ);
