...
import "./v25TellorTransfer.sol";


/**
* @title v25Tellor Dispute
* @dev Contains the methods related to disputes. v25Tellor.sol references this library for function's logic.
*/

library v25TellorDispute {
    using v25SafeMath for uint256;
    using v25SafeMath for int256;
    ...
    * when a miner returns a false on the validate array(in v25Tellor.ProofOfWork) it sends the
    ...
    */
    function beginDispute(v25TellorStorage.TellorStorageStruct storage self, uint256 _requestId, uint256 _timestamp, uint256 _minerIndex) public {
        ...
        self.disputesById[disputeId] = v25TellorStorage.Dispute({
            hash: _hash,
            isPropFork: false,
        ...
        v25TellorTransfer.doTransfer(self, msg.sender, address(this),_fee);
    ...
    */
    function vote(v25TellorStorage.TellorStorageStruct storage self, uint256 _disputeId, bool _supportsDispute) public {
        ...
        uint256 voteWeight = v25TellorTransfer.balanceOfAt(self, msg.sender, disp.disputeUintVars[keccak256("blockNumber")]);
    ...
    * @param _disputeId is the dispute id
    */
    function tallyVotes(v25TellorStorage.TellorStorageStruct storage self, uint256 _disputeId) public {
        v25TellorStorage.Dispute storage disp = self.disputesById[_disputeId];
        ...
        if (disp.isPropFork == false) {
                //Ensure the time for voting has elapsed
                    v25TellorStorage.StakeInfo storage stakes = self.stakerDetails[disp.reportedMiner];
    ...
    * @param _propNewTellorAddress address for new proposed v25Tellor
    */
    function proposeFork(v25TellorStorage.TellorStorageStruct storage self, address _propNewTellorAddress) public {
        bytes32 _hash = keccak256(abi.encode(_propNewTellorAddress));
        v25TellorTransfer.doTransfer(self, msg.sender, address(this), 100e18); //This is the fork fee (just 100 tokens flat, no refunds)
    ...
    /**
    * @dev Updates the v25Tellor address after a proposed fork has 
    * passed the vote and day has gone by without a dispute
    * @param _disputeId the disputeId for the proposed fork
    */
    function updateTellor(v25TellorStorage.TellorStorageStruct storage self, uint _disputeId) public {
        ...
        v25TellorStorage.Dispute storage disp = self.disputesById[lastID];
    ...
    */
    function unlockDisputeFee (v25TellorStorage.TellorStorageStruct storage self, uint _disputeId) public {
        uint256 origID = self.disputeIdByDisputeHash[self.disputesById[_disputeId].hash];
        uint256 lastID =  self.disputesById[origID].disputeUintVars[keccak256(abi.encode(self.disputesById[origID].disputeUintVars[keccak256("disputeRounds")]))];
        if(lastID == 0){
            lastID = origID;
        }
        v25TellorStorage.Dispute storage disp = self.disputesById[origID];
        v25TellorStorage.Dispute storage last = self.disputesById[lastID];
        ...
        if (last.disputeVotePassed == true){
                ...
                if(stakes.currentStatus == 4){
                    ...
                    v25TellorTransfer.doTransfer(self,disp.reportedMiner,disp.reportingParty,self.uintVars[keccak256("stakeAmount")]);
                ...
                for(uint i = 0; i < dispRounds;i++){
                    ...
                    v25TellorStorage.Dispute storage disp2 = self.disputesById[_id];
                        //transfer fee adjusted based on number of miners if the minerIndex is not 2(official value)
                    v25TellorTransfer.doTransfer(self,address(this), disp2.reportingParty, disp2.disputeUintVars[keccak256("fee")]);
                }
            }
            else {
                stakes.currentStatus = 1;
                v25TellorStorage.Request storage _request = self.requestDetails[disp.disputeUintVars[keccak256("requestId")]];
                ...
                for(uint i = 0; i < dispRounds;i++){
                    ...
                    v25TellorTransfer.doTransfer(self,address(this),last.reportedMiner,self.disputesById[_id].disputeUintVars[keccak256("fee")]);
                }
            }
    ...
    * of staked miners
    */
    function updateMinDisputeFee(v25TellorStorage.TellorStorageStruct storage self) public {
        uint256 stakeAmount = self.uintVars[keccak256("stakeAmount")];
        uint256 targetMiners = self.uintVars[keccak256("targetMiners")];
        self.uintVars[keccak256("disputeFee")] = v25SafeMath.max(15e18,
                (stakeAmount-(stakeAmount*(v25SafeMath.min(targetMiners,self.uintVars[keccak256("stakerCount")])*1000)/
                targetMiners)/1000));
    }
}
