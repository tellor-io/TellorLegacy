...
import "./v2TellorTransfer.sol";
import "./v2TellorDispute.sol";
import "./v2TellorStake.sol";
import "./v2TellorGettersLibrary.sol";
...
library v2TellorLibrary {
    ...
    */
    function addTip(v2TellorStorage.TellorStorageStruct storage self, uint256 _requestId, uint256 _tip) public {
        ...
        v2TellorTransfer.doTransfer(self, msg.sender, address(this), _tip);
    ...
    */
    function newBlock(v2TellorStorage.TellorStorageStruct storage self, string memory _nonce, uint256[5] memory _requestId) public {
        v2TellorStorage.Request storage _tblock = self.requestDetails[self.uintVars[_tBlock]];
        ...
        int256 _change = int256(v2SafeMath.min(1200, (now - self.uintVars[timeOfLastNewValue])));
        int256 _diff = int256(self.uintVars[difficulty]);
        _change = (_diff * (int256(self.uintVars[timeTarget]) - _change)) / 4000;
        if (_change == 0) {
                _change = 1;
            }
        self.uintVars[difficulty]  = uint256(v2SafeMath.max(_diff + _change,1));
        ...
        for (uint k = 0; k < 5; k++) {
            ...
            v2TellorStorage.Request storage _request = self.requestDetails[_requestId[k]];
        ...
        v2TellorTransfer.doTransfer(self, address(this), self.addressVars[_owner],  _devShare);
        self.uintVars[_tBlock] ++;
        self.uintVars[currentTotalTips] = 0;
        uint256[5] memory _topId = v2TellorStake.getTopRequestIDs(self);
        for(uint i = 0; i< 5;i++){
...
/**
    ...
    * @param _requestId for the current request being mined
    */
    function newBlock(v2TellorStorage.TellorStorageStruct storage self, string memory _nonce, uint256 _requestId) public {
       ...
       int256 _diff = int256(self.uintVars[difficulty]);
        ...
        v2TellorStorage.Details[5] memory a = self.currentMiners;
        uint256 i;
        for (i = 1; i < 5; i++) {
        ...
        for (i = 0; i < 5; i++) {
            v2TellorTransfer.doTransfer(self, address(this), a[i].miner, self.uintVars[currentReward]  + self.uintVars[currentTotalTips] / 5);
        }
        //update the total supply
        self.uintVars[total_supply] +=  self.uintVars[devShare] + self.uintVars[currentReward]*5 ;
        //pay the dev-share
        v2TellorTransfer.doTransfer(self, address(this), self.addressVars[_owner],  self.uintVars[devShare]);
        //Save the official(finalValue), timestamp of it, 5 miners and their submitted values for it, and its block number
        ...
        if(self.uintVars[timeTarget] == 600){
            ...
            self.uintVars[difficulty] = v2SafeMath.max(1,self.uintVars[difficulty]/3);
        }
        for(i = 0; i< 5;i++){
    ...
    */
    function submitMiningSolution(v2TellorStorage.TellorStorageStruct storage self, string memory _nonce, uint256 _requestId, uint256 _value)
        public
    {
    ...
    */
    function submitMiningSolution(v2TellorStorage.TellorStorageStruct storage self, string calldata _nonce,uint256[5] calldata _requestId, uint256[5] calldata _value)
        external
    {
    ...
    */
    function _payReward(v2TellorStorage.TellorStorageStruct storage self, uint _slotProgress) internal {
        ...
        v2TellorTransfer.doTransfer(self, address(this), msg.sender, self.uintVars[currentReward]  + _runningTips / 2 / 5 + _extraTip);
    ...
    */
    function proposeOwnership(v2TellorStorage.TellorStorageStruct storage self, address payable _pendingOwner) public {
    ...
    */
    function claimOwnership(v2TellorStorage.TellorStorageStruct storage self) public {
    ...
    */
    function updateOnDeck(v2TellorStorage.TellorStorageStruct storage self, uint256 _requestId, uint256 _tip) public {
        v2TellorStorage.Request storage _request = self.requestDetails[_requestId];
        ...
        if(self.currentMiners[0].value == _requestId || self.currentMiners[1].value== _requestId ||self.currentMiners[2].value == _requestId||self.currentMiners[3].value== _requestId || self.currentMiners[4].value== _requestId ){
            ...
            if (_request.apiUintVars[requestQPosition] == 0) {
                uint256 _min;
                uint256 _index;
                (_min, _index) = v2Utilities.getMin(self.requestQ);
