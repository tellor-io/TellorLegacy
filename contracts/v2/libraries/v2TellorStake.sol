...
library v2TellorStake {
    ...
    */
    function init(v2TellorStorage.TellorStorageStruct storage self) public {
        ...
        v2TellorTransfer.updateBalanceAtNow(self.balances[address(this)], 2**256 - 1 - 6000e18);
        ...
        for (uint256 i = 0; i < 6; i++) {
            ...
            v2TellorTransfer.updateBalanceAtNow(self.balances[_initalMiners[i]], 1000e18);
    ...
    */
    function requestStakingWithdraw(v2TellorStorage.TellorStorageStruct storage self) public {
        v2TellorStorage.StakeInfo storage stakes = self.stakerDetails[msg.sender];
        ...
        v2TellorDispute.updateMinDisputeFee(self);
    ...
    */
    function withdrawStake(v2TellorStorage.TellorStorageStruct storage self) public {
        v2TellorStorage.StakeInfo storage stakes = self.stakerDetails[msg.sender];
    ...
    */
    function depositStake(v2TellorStorage.TellorStorageStruct storage self) public {
    ...
    */
    function newStake(v2TellorStorage.TellorStorageStruct storage self, address staker) internal {
        require(v2TellorTransfer.balanceOf(self, staker) >= self.uintVars[keccak256("stakeAmount")], "Balance is lower than stake amount");
        ...
        self.uintVars[keccak256("stakerCount")] += 1;
        self.stakerDetails[staker] = v2TellorStorage.StakeInfo({
    ...
    */
    function getNewCurrentVariables(v2TellorStorage.TellorStorageStruct storage self) internal view returns(bytes32 _challenge,uint[5] memory _requestIds,uint256 _difficulty, uint256 _tip){
        for(uint i=0;i<5;i++){
    ...
    */
    function getNewVariablesOnDeck(v2TellorStorage.TellorStorageStruct storage self) internal view returns (uint256[5] memory idsOnDeck, uint256[5] memory tipsOnDeck) {
    ...
    */
    function getTopRequestIDs(v2TellorStorage.TellorStorageStruct storage self) internal view returns (uint256[5] memory _requestIds) {
        uint256[5] memory _max;
        uint256[5] memory _index;
        (_max, _index) = v2Utilities.getMax5(self.requestQ);
        for(uint i=0;i<5;i++){
            if(_max[i] != 0){
