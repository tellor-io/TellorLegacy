...
library v2TellorDispute {
    using v2SafeMath for uint256;
    using v2SafeMath for int256;
    ...
    */
    function beginDispute(v2TellorStorage.TellorStorageStruct storage self, uint256 _requestId, uint256 _timestamp, uint256 _minerIndex) public {
        ...
        self.disputesById[disputeId] = v2TellorStorage.Dispute({
            hash: _hash,
            isPropFork: false,
        ...
        v2TellorTransfer.doTransfer(self, msg.sender, address(this),_fee);
    ...
    */
    function vote(v2TellorStorage.TellorStorageStruct storage self, uint256 _disputeId, bool _supportsDispute) public {
        ...
        uint256 voteWeight = v2TellorTransfer.balanceOfAt(self, msg.sender, disp.disputeUintVars[keccak256("blockNumber")]);
    ...
    * @param _disputeId is the dispute id
    */
    function tallyVotes(v2TellorStorage.TellorStorageStruct storage self, uint256 _disputeId) public {
        v2TellorStorage.Dispute storage disp = self.disputesById[_disputeId];
        ...
        if (disp.isPropFork == false) {
                //Ensure the time for voting has elapsed
                    v2TellorStorage.StakeInfo storage stakes = self.stakerDetails[disp.reportedMiner];
    ...
    */
    function proposeFork(v2TellorStorage.TellorStorageStruct storage self, address _propNewTellorAddress) public {
        bytes32 _hash = keccak256(abi.encode(_propNewTellorAddress));
        v2TellorTransfer.doTransfer(self, msg.sender, address(this), 100e18); //This is the fork fee (just 100 tokens flat, no refunds)
    ...
    */
    function updateTellor(v2TellorStorage.TellorStorageStruct storage self, uint _disputeId) public {
        ...
        v2TellorStorage.Dispute storage disp = self.disputesById[lastID];
    ...
    */
    function unlockDisputeFee (v2TellorStorage.TellorStorageStruct storage self, uint _disputeId) public {
        uint256 origID = self.disputeIdByDisputeHash[self.disputesById[_disputeId].hash];
        uint256 lastID =  self.disputesById[origID].disputeUintVars[keccak256(abi.encode(self.disputesById[origID].disputeUintVars[keccak256("disputeRounds")]))];
        if(lastID == 0){
            lastID = origID;
        }
        v2TellorStorage.Dispute storage disp = self.disputesById[origID];
        v2TellorStorage.Dispute storage last = self.disputesById[lastID];
        ...
        if (last.disputeVotePassed == true){
                ...
                if(stakes.currentStatus == 4){
                    ...
                    v2TellorTransfer.doTransfer(self,disp.reportedMiner,disp.reportingParty,self.uintVars[keccak256("stakeAmount")]);
                ...
                for(uint i = 0; i < dispRounds;i++){
                    ...
                    v2TellorStorage.Dispute storage disp2 = self.disputesById[_id];
                        //transfer fee adjusted based on number of miners if the minerIndex is not 2(official value)
                    v2TellorTransfer.doTransfer(self,address(this), disp2.reportingParty, disp2.disputeUintVars[keccak256("fee")]);
                }
            }
            else {
                stakes.currentStatus = 1;
                v2TellorStorage.Request storage _request = self.requestDetails[disp.disputeUintVars[keccak256("requestId")]];
                ...
                for(uint i = 0; i < dispRounds;i++){
                    ...
                    v2TellorTransfer.doTransfer(self,address(this),last.reportedMiner,self.disputesById[_id].disputeUintVars[keccak256("fee")]);
                }
            }
    ...
    * of staked miners
    */
    function updateMinDisputeFee(v2TellorStorage.TellorStorageStruct storage self) public {
        uint256 stakeAmount = self.uintVars[keccak256("stakeAmount")];
        uint256 targetMiners = self.uintVars[keccak256("targetMiners")];
        self.uintVars[keccak256("disputeFee")] = v2SafeMath.max(15e18,
                (stakeAmount-(stakeAmount*(v2SafeMath.min(targetMiners,self.uintVars[keccak256("stakerCount")])*1000)/
                targetMiners)/1000));
    }
}
